...
const Dashboard = () => {
    ...
    return (
        <>
            <h2 className={style.title}>Explore Public Assets</h2>

            {assets.length === 0 ? (
                <div className={style.empty}>No public assets found</div>
            ) : (
                <div className={style.grid}>
                    {assets.map((asset) => (
                        <Card
                            ...
                            title={asset.title}
                            subtitle={asset.description || "No description"}
                            author={
                                asset.owner?.name
                                    ? `By ${asset.owner.name}`
                                    : "Unknown author"
